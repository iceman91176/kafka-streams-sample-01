...
curl -X POST \
  http://schema-registry:8081/subjects/generic_location-value/versions \
...
sed src/main/resources/avro/$schemaname.avsc -e 's/"/\\"/g' | tr '\n' ' ' | sed '1s/^/{"schema":"/' | sed '$ s/$/"}/' > /tmp/$schemaname.json
curl -X POST \
  http://schema-registry:8081/subjects/generic_location_contact-value/versions \
  -H 'Cache-Control: no-cache' \
  -H 'Content-Type: application/vnd.schemaregistry.v1+json' \
  --data-binary "@/tmp/$schemaname.json"
 rm -rf /tmp/$schemaname.json
...
sed src/main/resources/avro/$schemaname.avsc -e 's/"/\\"/g' | tr '\n' ' ' | sed '1s/^/{"schema":"/' | sed '$ s/$/"}/' > /tmp/$schemaname.json
curl -X POST \
  http://schema-registry:8081/subjects/generic_contact-value/versions \
  -H 'Cache-Control: no-cache' \
  -H 'Content-Type: application/vnd.schemaregistry.v1+json' \
  --data-binary "@/tmp/$schemaname.json"
 rm -rf /tmp/$schemaname.json